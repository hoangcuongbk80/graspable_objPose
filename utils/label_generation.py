...
def process_grasp_labels(end_points):
    ...
    batch_grasp_view_graspability = torch.sum(torch.sum(batch_grasp_view_valid, dim=-1), dim=-1) / view_grasp_num  # (B, Ns, V)
    view_graspability_min, _ = torch.min(batch_grasp_view_graspability, dim=-1)  # (B, Ns)
    view_graspability_max, _ = torch.max(batch_grasp_view_graspability, dim=-1)
    view_graspability_max = view_graspability_max.unsqueeze(-1).expand(-1, -1, 300)  # (B, Ns, V)
    view_graspability_min = view_graspability_min.unsqueeze(-1).expand(-1, -1, 300)  # same shape as batch_grasp_view_graspability
    batch_grasp_view_graspability = (batch_grasp_view_graspability - view_graspability_min) / (view_graspability_max - view_graspability_min + 1e-5)
    ...
    end_points['batch_grasp_view_graspability'] = batch_grasp_view_graspability
